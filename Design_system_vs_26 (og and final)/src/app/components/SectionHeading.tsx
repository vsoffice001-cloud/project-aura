    ...
    <div className={`${spacingClass} ${isCenter ? 'text-center' : ''}`}>
      {(label || labelEndSlot) && (
        <div className="flex items-center justify-between mb-2">
          {label && (
            <p
              className="tracking-[0.15em] uppercase flex items-center gap-2 opacity-35"
      ...
      >
        <div className={widthClass}>
          ...
          {subtitle && (
            <p
              className={`mt-2 leading-relaxed opacity-40 ${isCenter ? 'mx-auto' : ''}`}
              style={{ fontSize: 'var(--text-xs)' }}
            >
              {subtitle}
            </p>
        ...
        {(action || endSlot) && (
          <div className="flex items-center gap-3 flex-shrink-0">
            ...
            {action && (
              <div className="flex">
                <CTALink size="md" href={action.href || "#"} onClick={action.onClick}>
                  {action.text}
                </CTALink>
              </div>
            )}
