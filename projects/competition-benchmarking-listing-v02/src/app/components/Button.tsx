import { type CSSProperties, type MouseEvent, type ReactElement, ReactNode, useState, useRef, cloneElement, isValidElement } from 'react';
...
export function Button({
  ...
  const sizeInlineStyles: Record<ButtonSize, CSSProperties> = {
    xs: { borderRadius: 'var(--radius-element)', ...(iconOnly ? {} : { paddingInline: '0.75rem' }) },
    sm: {
  ...
  const IconWrapper = ({ children: iconChild }: { children: ReactNode }) => {
    if (isValidElement(iconChild)) {
      return cloneElement(iconChild as ReactElement<any>, {
        size: iconSize,
        strokeWidth: 2,
  ...
  return (
    ...
    >
      ...
      {/* Focus Ring */}
      <span className="absolute inset-0 ring-2 ring-black ring-offset-2 opacity-0 focus-visible:opacity-100 pointer-events-none" style={{ borderRadius: 'var(--radius-element)' }} />
    </button>
  );
}