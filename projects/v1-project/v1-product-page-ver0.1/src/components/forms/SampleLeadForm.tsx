...
/**
 * SampleLeadForm — PRD §46 "sample" form
 * Fields: full name, work email, company, country, use-case
 */
...
export function SampleLeadForm({ context, onClose }: Props) {
  ...
  if (state.submitted) {
    return <FormSuccess message="Thanks — sample report on its way to your inbox." />;
  }
  ...
  return (
    <form onSubmit={handleSubmit} noValidate className="flex flex-col gap-4">
      ...
      <FormTextarea
        label="What will you use this research for?"
        maxLength={250}
        value={state.useCase}
        onFocus={handleFirstFocus}
        onChange={(e) => dispatch({ type: 'SET_FIELD', field: 'useCase', value: e.target.value })}
        placeholder="e.g. market entry strategy, investor briefing…"
      />

      <SubmitButton label="Get Sample" loading={state.loading} />
      <LegalText />
    </form>
  );
}
